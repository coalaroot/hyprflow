...
Hyprflow is a dotfiles repository for a Hyprland Wayland compositor setup, structured for GNU Stow deployment. Each top-level directory (`hypr/`, `kitty/`, `rofi/`, `waybar/`) mirrors the path relative to `$HOME`, so `stow <package>` symlinks configs into place.
